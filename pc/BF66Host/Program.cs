...
#pet{--chest-color:#55e7ff;--chest-glow:#3bcbff;position:absolute;inset:0;display:none;overflow:hidden;touch-action:none;user-select:none;background:radial-gradient(circle at 50% 48%,#1b3473 0,#0b1731 43%,#050a17 100%);color:#eaf6ff}
.pet-grid{position:absolute;inset:0;opacity:.22;background-image:linear-gradient(rgba(77,130,255,.12) 1px,transparent 1px),linear-gradient(90deg,rgba(77,130,255,.12) 1px,transparent 1px);background-size:34px 34px;transform:perspective(380px) rotateX(64deg) scale(1.55);transform-origin:50% 88%}
.pet-halo{position:absolute;left:50%;top:59%;width:min(80vw,460px);height:min(80vw,460px);transform:translate(-50%,-50%);border-radius:50%;background:radial-gradient(circle,rgba(73,133,255,.26),rgba(97,69,255,.07) 50%,transparent 70%);filter:blur(6px);animation:halo 4.2s ease-in-out infinite}
#pet-bubble-zone{position:absolute;z-index:20;left:7%;right:7%;top:6%;height:20%;display:flex;align-items:flex-start;justify-content:center;pointer-events:none}
#pet-bubble{max-width:100%;max-height:100%;overflow:hidden;padding:12px 19px;border:1px solid rgba(126,180,255,.48);border-radius:18px;background:rgba(8,20,48,.9);backdrop-filter:blur(8px);font-size:20px;font-weight:650;line-height:1.35;text-align:center;opacity:0;transform:translateY(-8px);transition:opacity .2s,transform .2s;box-shadow:0 10px 28px rgba(0,0,0,.26)}#pet-bubble.show{opacity:1;transform:translateY(0)}
#pet-anchor{--drag-x:0px;--drag-y:0px;--tilt:0deg;position:absolute;z-index:6;left:50%;top:59%;width:min(57vw,330px);aspect-ratio:320/430;transform:translate(calc(-50% + var(--drag-x)),calc(-50% + var(--drag-y))) rotate(var(--tilt));cursor:pointer;outline:none;-webkit-tap-highlight-color:transparent;-webkit-touch-callout:none;will-change:transform}
...
#buddy{display:block;width:100%;height:100%;overflow:visible;filter:drop-shadow(0 22px 20px rgba(0,0,0,.42));pointer-events:none}.joint{transform-box:fill-box}.buddy-eye{fill:none;stroke:#65ecff;stroke-width:11;stroke-linecap:round;stroke-linejoin:round;filter:url(#eyeGlow)}#eyes-happy,#eyes-sleep,#eyes-surprise{display:none}
#buddy-root{transform-box:fill-box;transform-origin:50% 78%;animation:buddyBreathe 3.4s ease-in-out infinite}#buddy-head{transform-box:fill-box;transform-origin:50% 90%;animation:headObserve 9s ease-in-out infinite}#arm-left,#arm-right,#leg-left,#leg-right{transform-box:fill-box}#arm-left{transform-origin:86% 12%}#arm-right{transform-origin:14% 12%}#leg-left,#leg-right{transform-origin:50% 12%}
#chest-light{fill:var(--chest-color);filter:drop-shadow(0 0 8px var(--chest-glow));transition:fill .45s}.chest-bar{fill:#eefcff}.work-dot{animation:workDot 1.05s ease-in-out infinite}.work-dot:nth-child(2){animation-delay:.18s}.work-dot:nth-child(3){animation-delay:.36s}
#pet-anchor.working #buddy-head{animation:workNod 1.8s ease-in-out infinite}#pet-anchor.working #arm-left{animation:listenArmL 1.8s ease-in-out infinite}#pet-anchor.working #arm-right{animation:listenArmR 1.8s ease-in-out infinite}
#pet-anchor.wave #arm-right{animation:waveArm .34s ease-in-out 5}#pet-anchor.wave #buddy-head{animation:waveHead .7s ease-in-out 2}#pet-anchor.wave #eyes-normal{display:none}#pet-anchor.wave #eyes-happy{display:block}
#pet-anchor.jump #buddy-root{animation:buddyJump .85s cubic-bezier(.2,.8,.25,1)}#pet-anchor.jump #arm-left{animation:jumpArmL .85s ease}#pet-anchor.jump #arm-right{animation:jumpArmR .85s ease}#pet-anchor.jump #leg-left{animation:jumpLegL .85s ease}#pet-anchor.jump #leg-right{animation:jumpLegR .85s ease}#pet-anchor.jump #eyes-normal{display:none}#pet-anchor.jump #eyes-happy{display:block}
#pet-anchor.startle #buddy-root{animation:buddyStartle .7s ease}#pet-anchor.startle #arm-left{animation:startleArmL .7s ease}#pet-anchor.startle #arm-right{animation:startleArmR .7s ease}#pet-anchor.startle #eyes-normal{display:none}#pet-anchor.startle #eyes-surprise{display:block}
#pet-anchor.rest #buddy-root{animation:buddyRest 2.8s ease-in-out infinite}#pet-anchor.rest #buddy-head{animation:restHead 2.8s ease-in-out infinite}#pet-anchor.rest #arm-left{transform:rotate(22deg)}#pet-anchor.rest #arm-right{transform:rotate(-22deg)}#pet-anchor.rest #leg-left{transform:translate(16px,-7px) rotate(58deg)}#pet-anchor.rest #leg-right{transform:translate(-16px,-7px) rotate(-58deg)}#pet-anchor.rest #eyes-normal{display:none}#pet-anchor.rest #eyes-sleep{display:block}
#pet-anchor.dragging{transition:none}#pet-anchor.dragging #buddy-root{animation:none}#pet-anchor.drag-right #buddy-root{transform:rotate(8deg)}#pet-anchor.drag-left #buddy-root{transform:rotate(-8deg)}#pet-anchor.drag-right #arm-left,#pet-anchor.drag-right #arm-right{transform:rotate(-46deg)}#pet-anchor.drag-left #arm-left,#pet-anchor.drag-left #arm-right{transform:rotate(46deg)}#pet-anchor.drag-right #leg-left,#pet-anchor.drag-right #leg-right{transform:rotate(-24deg)}#pet-anchor.drag-left #leg-left,#pet-anchor.drag-left #leg-right{transform:rotate(24deg)}
#pet-anchor.land #buddy-root{animation:softLand .62s ease}#pet.usage-pulse #chest-light{animation:usagePulse .78s ease}
#pet-hint{position:absolute;z-index:8;left:0;right:0;bottom:5.4%;text-align:center;color:rgba(195,220,255,.62);font-size:13px;letter-spacing:.3px}.pet-status{position:absolute;z-index:9;right:14px;bottom:12px;padding:5px 9px;border:1px solid rgba(117,157,225,.3);border-radius:99px;color:#8ca8d5;font-size:10px;letter-spacing:.7px}
...
@keyframes buddyBreathe{0%,100%{transform:translateY(0) scale(1)}50%{transform:translateY(-5px) scale(1.012)}}@keyframes headObserve{0%,20%,100%{transform:rotate(0)}40%{transform:rotate(-3deg)}65%{transform:rotate(3deg)}}@keyframes halo{0%,100%{opacity:.6;transform:translate(-50%,-50%) scale(.94)}50%{opacity:1;transform:translate(-50%,-50%) scale(1.06)}}
@keyframes workNod{0%,100%{transform:rotate(-2deg) translateY(0)}50%{transform:rotate(2deg) translateY(2px)}}@keyframes listenArmL{0%,100%{transform:rotate(-8deg)}50%{transform:rotate(-16deg)}}@keyframes listenArmR{0%,100%{transform:rotate(8deg)}50%{transform:rotate(16deg)}}@keyframes workDot{0%,100%{opacity:.25}50%{opacity:1}}
@keyframes waveArm{0%,100%{transform:rotate(0)}50%{transform:translate(-9px,-12px) rotate(-72deg)}}@keyframes waveHead{0%,100%{transform:rotate(0)}50%{transform:rotate(-6deg)}}
@keyframes buddyJump{0%,100%{transform:translateY(0) scale(1)}30%{transform:translateY(12px) scale(1.04,.93)}55%{transform:translateY(-55px) scale(.97,1.05)}78%{transform:translateY(6px) scale(1.04,.94)}}@keyframes jumpArmL{35%,70%{transform:rotate(-82deg)}}@keyframes jumpArmR{35%,70%{transform:rotate(82deg)}}@keyframes jumpLegL{45%,72%{transform:rotate(30deg) translateY(-5px)}}@keyframes jumpLegR{45%,72%{transform:rotate(-30deg) translateY(-5px)}}
@keyframes buddyStartle{0%,100%{transform:translateY(0) scale(1)}25%{transform:translateY(-22px) scale(1.07)}55%{transform:translateY(5px) scale(.98)}}@keyframes startleArmL{25%,60%{transform:rotate(-78deg)}}@keyframes startleArmR{25%,60%{transform:rotate(78deg)}}
@keyframes buddyRest{0%,100%{transform:translateY(28px) scale(.95)}50%{transform:translateY(32px) scale(.94,.96)}}@keyframes restHead{0%,100%{transform:rotate(5deg)}50%{transform:rotate(8deg)}}@keyframes softLand{0%,100%{transform:translateY(0) scale(1)}35%{transform:translateY(9px) scale(1.06,.91)}65%{transform:translateY(-4px) scale(.98,1.03)}}@keyframes usagePulse{0%,100%{opacity:1;filter:drop-shadow(0 0 8px var(--chest-glow))}50%{opacity:.55;filter:drop-shadow(0 0 22px var(--chest-glow))}}@keyframes spark{0%{opacity:1;transform:translate(-50%,-20%) scale(.6)}100%{opacity:0;transform:translate(var(--dx),var(--dy)) scale(1.35)}}
...
#pet-bubble-zone{left:4%;right:auto;top:19%;width:38%;height:52%;align-items:center}#pet-bubble{font-size:16px;padding:10px 15px}#pet-anchor{left:68%;top:52%;width:min(32vw,270px)}.pet-halo{left:68%;top:52%;width:min(48vw,390px);height:min(48vw,390px)}#pet-hint{left:49%;bottom:2.5%;font-size:11px}.pet-status{top:9px;bottom:auto}
}
...
<section id="pet"><div class="pet-grid"></div><div class="pet-halo"></div><div id="pet-bubble-zone"><div id="pet-bubble"></div></div><div id="pet-anchor" aria-label="Codex 工作伙伴">
...
</g></svg></div><div id="pet-hint">轻触回应 · 拖动伙伴 · 长按休息</div><div class="pet-status" id="pet-transport">COMPANION</div></section>
...
function tick(){const d=new Date();$('clock').textContent=d.toLocaleTimeString('zh-CN',{hour:'2-digit',minute:'2-digit',hour12:false});}tick();setInterval(tick,1000);
...
const pet=$('pet'),petAnchor=$('pet-anchor'),petBubble=$('pet-bubble');let bubbleTimer=0,poseTimer=0,pressTimer=0,lastTap=0,longPressed=false,dragging=false,startX=0,startY=0,downAt=0,dragX=0,dragY=0,manualUntil=0,partnerState='',lastActivity='',lastWorkBubble=0,lastUsageTotal=null;
function petVisible(){return pet.style.display==='flex'}
function petSay(text,duration=1700){if(!petVisible())return;clearTimeout(bubbleTimer);petBubble.textContent=text;petBubble.classList.add('show');bubbleTimer=setTimeout(()=>petBubble.classList.remove('show'),duration)}
function removeTransient(){petAnchor.classList.remove('wave','jump','startle','land')}
function applyPartnerState(){if(Date.now()<manualUntil)return;petAnchor.classList.remove('working','rest');if(partnerState==='working')petAnchor.classList.add('working');if(partnerState==='rest')petAnchor.classList.add('rest')}
function sparks(){for(let i=0;i<8;i++){const p=document.createElement('span');p.className='spark';p.textContent=i%2?'✦':'●';p.style.color=i%2?'#7de7ff':'#927dff';p.style.setProperty('--dx',(Math.random()*250-125)+'px');p.style.setProperty('--dy',(-65-Math.random()*180)+'px');pet.appendChild(p);setTimeout(()=>p.remove(),950)}}
function petReact(kind,text,duration=1450){clearTimeout(poseTimer);removeTransient();petAnchor.classList.remove('working','rest');void petAnchor.offsetWidth;petAnchor.classList.add(kind);manualUntil=Date.now()+duration;if(text)petSay(text,Math.min(duration+250,1900));if(kind==='jump')sparks();poseTimer=setTimeout(()=>{removeTransient();applyPartnerState()},duration)}
function updateChest(u){if(!u)return;const left=u.hasWeekly?Number(u.remainingPercent):100;const color=left>=50?'#55e7ff':left>=20?'#ffd166':'#ff7085';const glow=left>=50?'#3bcbff':left>=20?'#ffb82e':'#ff4264';pet.style.setProperty('--chest-color',color);pet.style.setProperty('--chest-glow',glow);const total=Number(u.total||0);if(lastUsageTotal!==null&&total!==lastUsageTotal){pet.classList.remove('usage-pulse');void pet.offsetWidth;pet.classList.add('usage-pulse');setTimeout(()=>pet.classList.remove('usage-pulse'),850)}lastUsageTotal=total}
function updatePartner(u){if(!u)return;updateChest(u);const activity=u.lastActivityAt?Date.parse(u.lastActivityAt):0;const now=Date.now(),age=activity?now-activity:1e12;const next=age>10*60*1000?'rest':age<90*1000?'working':'standby';const changedActivity=activity&&lastActivity&&activity>Number(lastActivity);if(changedActivity&&now>=manualUntil&&now-lastWorkBubble>45000){lastWorkBubble=now;petSay('收到新的 Codex 工作。',1600)}if(next!==partnerState){const before=partnerState;partnerState=next;if(now>=manualUntil){applyPartnerState();if(before&&next==='rest')petSay('暂时没有新会话，休息一下。',1800);else if(before==='rest'&&next==='working')petSay('新任务已收到。',1500)}}lastActivity=String(activity||'')}
function manualRest(){clearTimeout(poseTimer);removeTransient();petAnchor.classList.remove('working');petAnchor.classList.add('rest');manualUntil=Date.now()+4200;petSay('暂停待命。',1600);poseTimer=setTimeout(()=>{petAnchor.classList.remove('rest');manualUntil=0;applyPartnerState()},4200)}
function returnHome(){dragging=false;petAnchor.classList.remove('dragging','drag-left','drag-right');petAnchor.classList.add('returning');petAnchor.style.setProperty('--drag-x','0px');petAnchor.style.setProperty('--drag-y','0px');petAnchor.style.setProperty('--tilt','0deg');setTimeout(()=>{petAnchor.classList.remove('returning');petReact('land','',620)},680)}
...
petAnchor.addEventListener('pointermove',e=>{if(!petAnchor.hasPointerCapture(e.pointerId))return;const dx=e.clientX-startX,dy=e.clientY-startY;if(!dragging&&Math.hypot(dx,dy)>9){dragging=true;clearTimeout(pressTimer);petAnchor.classList.remove('returning','working','rest');petAnchor.classList.add('dragging')}if(!dragging)return;dragX=Math.max(-78,Math.min(78,dx));dragY=Math.max(-48,Math.min(48,dy));petAnchor.style.setProperty('--drag-x',dragX+'px');petAnchor.style.setProperty('--drag-y',dragY+'px');petAnchor.style.setProperty('--tilt',Math.max(-7,Math.min(7,dx/12))+'deg');petAnchor.classList.toggle('drag-right',dx>3);petAnchor.classList.toggle('drag-left',dx<-3)});
petAnchor.addEventListener('pointerup',e=>{e.preventDefault();clearTimeout(pressTimer);if(dragging){const quick=Date.now()-downAt<520&&Math.abs(dragX)>58;returnHome();if(quick)petSay('重新就位。',1200);return}if(longPressed)return;const now=Date.now();if(now-lastTap<420){lastTap=0;petReact('jump','任务能量 +1',1150)}else{lastTap=now;const rect=petAnchor.getBoundingClientRect(),head=(startY-rect.top)/rect.height<.57;setTimeout(()=>{if(lastTap===now){petReact(head?'wave':'startle',head?'我在听。':'工作状态正常。',1450);lastTap=0}},430)}});
